...
let runnerId = [];
runnerID = runners.filter (item => item.id < 50);
console.log(runnerID);

// Problem 3
// Predicting twice the amount of donations next year return total * 2
//AND IM TOTALLY STUCK ON THIS NVM GOT IT HAHA
let ticketPriceTotalDoubled = [];
ticketPriceTotalDouble = runners.reduce ((tracker, item) => tracker + item.donation*2,0);
console.log(ticketPriceTotalDouble);